...
contract('ContractRegistry', (accounts) => {
  ...
  it("should allow admin to register a contract", async () => {
    ...
    assert.equal(register.logs[0].args.name, "GameJam", "Incorrect contract name stored")
    assert.equal(register.logs[0].args.contractAddress, gameJam.address, "Incorrect contract address stored")
  });

  it("should allow admin to be a competitor", async () => {
    await testWillThrow(contractRegistry.updateContractAddress,  ["GameJam", admin], { from: pleb }, "Admin role required")
  });
  ...
  it("a pleb user should get a contract address", async () => {
    ...
    assert.equal(address, gameJam.address, "Incorrect contract address stored")
  });
});